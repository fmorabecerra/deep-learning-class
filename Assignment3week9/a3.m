% Only scripted provided by class. There is only one function that you are supposed to modify
% That function is d_loss_by_d_model. Check out comments there to see what you did.
% Also, I commented out the test_gradient function. Really takes up alot of time. not nessesary unless you want to check 
% that you are doing things the right way at first.
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % PANCHO'S WORK BELOW %%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % This stuff right here was snatched from the loss function
  % Just have to make sure that I forward propagate first.
  % Before we can calculate the loss, we need to calculate a variety of intermediate values, like the state of the hidden units.
  hid_input = model.input_to_hid * data.inputs; % input to the hidden units, i.e. before the logistic. size: <number of hidden units> by <number of data cases>
  hid_output = logistic(hid_input); % output of the hidden units, i.e. after the logistic. size: <number of hidden units> by <number of data cases>
  class_input = model.hid_to_class * hid_output; % input to the components of the softmax. size: <number of classes, i.e. 10> by <number of data cases>
  % this is the softmax stuff
  class_normalizer = log_sum_exp_over_rows(class_input); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob); % probability of each class. Each column (i.e. each case) sums to 1. size: <number of classes, i.e. 10> by <number of data cases>
  
%  display(model.hid_to_class)
%  cost = loss(model, data, wd_coefficient)
%  display('Lets display the cost:')
%  display(cost)
%  display()

%  display('Size of input:')
%  display(size(data.inputs))
%  display(size(data.inputs,2))

%  display('size of weight decay coef')
%  display(size(wd_coefficient))
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % COMPUTE HIDDEN TO CLASS WEIGHT GRADIENTS
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Note: you have to divide by number of inputs. Some how this takes the average.
  % Origninal Answer: ret.hid_to_class = model.hid_to_class * 0;
  % the derivative for softmax layer is dC/dw = (y-t)*h.
  error_deriv_soft = (class_prob - data.targets);  % May have to switch this.
  %hid_to_output_weights_gradient =  hidden_layer_state * error_deriv';
  ret.hid_to_class = (hid_output * error_deriv_soft')'/size(data.inputs,2)+ wd_coefficient*model.hid_to_class; % this is what it had in the previous assignment.
  % Check to see that outputs are same dimensions
  %size(model.hid_to_class)
  %size(ret.hid_to_class)

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % COMPUTE INPUT TO HIDDEN WEIGHT GRADIENTS
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Original Answer: ret.input_to_hid = model.input_to_hid * 0;
  %back_propagated_deriv_1 = (hid_to_output_weights * error_deriv) ...
  %    .* hidden_layer_state .* (1 - hidden_layer_state);
  back_propagated_deriv_1 = ((model.hid_to_class' * error_deriv_soft) ...
      .* hid_output .* (1 - hid_output));
%  display('size of back_propagated_deriv_1:')
%  size(back_propagated_deriv_1)
  %embed_to_hid_weights_gradient = embedding_layer_state * back_propagated_deriv_1';
  ret.input_to_hid = (data.inputs * back_propagated_deriv_1')'/size(data.inputs,2)+ wd_coefficient*model.input_to_hid;
  % Check to see that outputs are same dimensions
%  display('check sizes of weights and gradient MTXs')
%  size(model.input_to_hid)
%  size(ret.input_to_hid)
%  display('Made it to the end of the function!!!')
end
