...
% This takes in the (binary) states of the visible units, and returns the activation probabilities of the hidden units conditional on those states.
    ...
    % Pancho: you just have to compute the dot product and then calculate the probabilities
    
    % Dot product:
    dot_product = rbm_w * visible_state;
    % Now let's compute the probabilities
    hidden_probability = 1 ./ (1+exp(-1*dot_product));
